...
const SUPPORTED_VARIABLES = ["tag", "props", "child", "key", "ref", "hooks"];
...
const validateFunctionAnswer = function (res, index) {
  if (res === undefined) {
    error(
      `${res} | номер в массиве: ${index} - ${errorMessage.functionReturnUndefinedOrNull}`,
    );
  }
  const type = typeOf(res);
  if (!CORRECT_ANSWER.includes(type)) {
    error(
      `${res} | номер в массиве: ${index} - ${errorMessage.functionReturnIncorrectData}`,
    );
  }
};
